...
만든 스킬을 **[`teamspace-first-skills`](https://github.com/teamspace-ai-all/teamspace-first-skills)** 레포에 PR로 제출한다. 이 레포는 신규입사자들의 첫 스킬이 모이는 갤러리다.
...
> "teamspace-first-skills 레포에 내 스킬 커밋하고 PR 만들어줘"

Claude가 알아서 다음을 수행합니다:

```bash
# 0. 제출 대상 레포 클론
git clone https://github.com/teamspace-ai-all/teamspace-first-skills.git
cd teamspace-first-skills
...
**4. Git: 브랜치 → 커밋 → PR**
Claude에게 "teamspace-first-skills 레포에 내 스킬 커밋하고 PR 만들어줘"라고 요청하면 전 과정을 도와줍니다.

**5. Slack에서 GitHub Notifications 앱 설치**
PR을 올렸으니, 리뷰 댓글이나 머지 알림을 Slack에서 받을 수 있도록 설정하세요.
- Slack 좌측 하단 > **앱** > "GitHub" 검색 > **추가**
- 추가 후 DM으로 `/github subscribe teamspace-ai-all/teamspace-first-skills` 입력하면 알림 연동 완료
