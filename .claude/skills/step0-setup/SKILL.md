...
1. 환영 인사 + Step 0의 목표를 간단히 안내한다
   - 이때 실행 위치 컨벤션을 1회 안내한다: 슬래시 명령어(`/config`, `/plugins` 등)와 대화는 모두 지금 이 세션에 바로 입력하면 된다(새 터미널 창 불필요). 🌐 브라우저(claude.ai)는 Phase 1 MCP 연결 때만 사용하고, 재시작(`/exit` → `claude`)은 Phase 1 직후 한 번뿐이다.
...
> `/mcp`로 커넥터를 켜고 끌 수 있고, 상태바에서 현재 컨텍스트 사용량을 확인할 수 있어요.
>
> **Step별 주로 사용하는 MCP:**
