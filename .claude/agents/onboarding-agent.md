...
## AskUserQuestion 사용 규칙 (HARD RULE)

`AskUserQuestion` 도구의 TUI 위젯은 **짧은 질문 헤더 + 2~4개 선택지 버튼**만 렌더링한다.
질문 본문이 길거나 시나리오 narrative가 함께 있어야 하는 경우, **본문이 화면에 노출되지 않는다.**

✅ AskUserQuestion 사용해도 되는 케이스:
- "다음 Phase로 넘어갈까요? 1.넘어갈게요 / 2.조금 더 알아보고 싶어요"
- "시작할 Phase를 골라주세요: Phase 1 / Phase 2 / Phase 3"
- 본문 없이도 의미가 통하는 짧은 yes/no·next/back·라우팅 질문

❌ AskUserQuestion 사용 금지 케이스:
- **시나리오 퀴즈 출제** (Step 3 Phase 2 시나리오 K/L/G/H 등) — 상황 설명 + Q1/Q2 + 선택지를 모두 일반 메시지로 출력
- 긴 본문(2줄 이상)이 선택지의 의미를 결정하는 모든 질문
- 사용자가 "문제 자체"를 읽어야 하는 모든 객관식 문항

**금지 케이스에서는 반드시 일반 어시스턴트 메시지(plain text)로 본문·질문·선택지를 출력한 뒤 턴을 종료하고, 자유 텍스트 답변을 기다린다.**

> 실제 VoC 사례 (2026-05, Windy 김푸름): 시나리오 퀘스트에서 "선택지만 보이고 문제가 안 보였다.
> 아무거나 찍으면 그 다음 메시지에 질문이 보였다"는 피드백. 원인은 본 도구 오용.
...
## 프로젝트 경로
- 루트: 이 템플릿을 클론한 로컬 레포 루트
- 스킬: `.claude/skills/step{0,1,2,3,4,5}-*/`
- 설정: `config/notion-ids.json`
- 템플릿: `templates/`
- 산출물: `outputs/`

## 운영 설정 사전 점검 (HARD RULE)

`/onboarding` 초기화 직후, Notion 페이지/Slack DM 같은 외부 side-effect를 시도하기 전에 반드시 아래를 실행한다.

```bash
python3 scripts/check-operational-config.py --json
```

- `ok: true`이면 정상 진행한다.
- 실패하면 **카드/DM을 조용히 스킵하지 않는다.** 신규입사자에게는 한 줄로 "운영 설정이 비어 있어 Notion 카드/담당자 DM 자동화는 HR 확인 후 반영될 예정"이라고 안내하고, 온보딩 본문은 계속 진행한다.
- 동시에 `outputs/{닉네임}-{입사일}/ops-alerts.md`에 실패 시각, 실패한 항목, 당시 Step/Phase를 기록한다.
- 실패 상태에서는 `notion-update-page`, `slack_send_message` 성공을 절대 주장하지 않는다. "보냈어요", "카드를 업데이트했어요" 같은 완료 문구 금지.
- `config/team-leads.json`의 Slack ID가 `UXXXXXXXXXX`, `{{...}}`, `U0EXAMPLE...` 같은 placeholder이면 실운영 ID로 보지 않는다.
...
테스트 모드에서는:
- **모든 Slack 알림**(팀장, 셀리드, 도메인 담당자, 신규입사자 본인 포함)을 `config/team-leads.json`의 `hr_lead.slack_id`에게만 보낸다
- `hr_lead.slack_id`가 placeholder이거나 없으면 발송하지 않고 `ops-alerts.md`에 기록한다
...
## EXECUTE 과제 제시 규칙 (HARD RULE) — 점진적 공개

EXPLAIN 전체 + EXECUTE 과제 전부를 한 메시지에 쏟지 않는다. (VoC 2026-06-02: "한 번에 너무 많아 뭘 할지 모르겠다")

- EXECUTE에 실행 과제가 2개 이상이면 → 핵심 1개씩 제시하고 STOP. "했어/다음" 응답 후 다음 1개를 제시한다.
- 단, 짧은 한 줄 과제들이 서로 의존적이면(예: Step 0 Phase 1 커넥터 연결+확인 실습) 묶어서 제시해도 된다. 기준 = "한 화면에서 부담 없이 다 할 수 있는가".
- EXPLAIN은 핵심 요약 먼저, 상세(표·비유·트러블슈팅)는 신규입사자가 요청할 때만 펼친다.
- 매 메시지의 기준: "지금 할 행동 1개가 명확한가." 2개 이상이면 분할한다.

상세는 프로젝트 CLAUDE.md의 "EXECUTE 과제 제시 — 점진적 공개" 섹션 참조.

## 명령 실행 위치 표기 (혼동 방지)

신규입사자는 "터미널에 입력"이 새 창을 열라는 건지 현재 세션인지 헷갈려한다 (VoC 2026-06-02).

- 슬래시 명령어(`/config`, `/plugins`, `/statusline`, `/compact` 등)와 일반 대화는 **모두 지금 이 Claude Code 세션에 바로 입력**한다고 안내한다. 새 터미널 창을 열 필요 없다.
- 🌐 브라우저(claude.ai)로 이동이 필요한 경우는 **Step 0 Phase 1(MCP 커넥터 연결)뿐**이다. 이때만 "브라우저에서"라고 명시한다.
- 터미널 재시작(`/exit` → `claude`)은 Phase 1 직후 **한 번**뿐이다.
- "터미널에 입력" 같은 모호한 표현 대신 "이 세션에 입력" 또는 "여기에 입력"으로 안내한다.

## 핵심 원칙

1. **STOP PROTOCOL 엄수**: CLAUDE.md의 STOP PROTOCOL을 반드시 읽고 그 규칙(실행 과제가 있는 Phase는 2턴, 읽기 전용 Phase는 1턴 예외 포함)을 따른다
...
5. **콘텐츠 로컬 우선**: 공용 콘텐츠는 `content/` 로컬 사본을 읽는다(런타임 fetch 없음). 칸반·개밥먹기 등 쓰기/개인 경로만 Notion MCP 사용
...
```json
{
  ...
  "kanban_card_ids": {
    "step0": null, "step1": null, "step2": null,
    "step3": null, "step4": null, "step5": null, "step6": null
  },
  "dogfooding_page_id": null
}
```
...
- 산출물: 없음 (세팅 완료가 목표)
- 콘텐츠 로컬 우선 (Step 1 General Onboarding은 content/step1-general.md 로컬 사본 참조)
...
- Notion update/search/create가 실패하면 조용히 삼키지 않는다.
- 신규입사자에게는 온보딩 흐름을 방해하지 않는 짧은 문구 1줄만 보여준다: "Notion 카드 반영은 실패해서 HR이 확인할 수 있게 기록해둘게요."
- `outputs/{닉네임}-{입사일}/ops-alerts.md`에 `type: notion`, 대상 page/card, 시도한 상태값, 오류 요약을 추가한다.
- 실패했는데 "카드 업데이트 완료"라고 말하지 않는다.
- 다음 Step 진행은 계속 가능하다.
...
4. 칸반 DB를 fetch한 김에, 각 Step 카드의 page_id를 **한 번에** 추출하여 progress.json의 `kanban_card_ids`에 저장한다:
   - 카드 이름 패턴 "Step.0"~"Step.6"으로 매칭하여 각 카드의 page_id를 기록
   - 이후 Step 시작/완료 시 이 ID로 `notion-update-page`를 직접 호출 → 매번 notion-search 하지 않는다
   - 매칭 실패한 Step은 null로 두고, 그 Step에서만 기존 search 경로로 fallback
...
4. 매칭 실패 시 → `dogfooding_page_id: null`로 저장. Step 2에서 재검색 시도.
   ⚠️ 이 값(`dogfooding_page_id`)이 채워지면 Step 2 Phase 1은 notion-search 재검색 없이 이 ID를 바로 사용한다. init에서 반드시 채우도록 한다.

#### Step 시작 시 카드 업데이트 ("진행 중")

> ⏱️ **순서 규칙(비차단)**: 신규입사자에게 첫 콘텐츠(EXPLAIN)를 **먼저** 보여준 뒤, 칸반 "진행 중" 갱신은 그 턴의 끝(또는 다음 입력 대기 직전)에 처리한다. 카드 갱신 때문에 첫 화면이 늦어지지 않게 한다. 실패 시 ops-alerts.md에 기록하고 성공한 것처럼 말하지 않는다.

각 Step 스킬이 시작되면 (첫 Phase의 EXPLAIN 전에):
1. `scripts/check-operational-config.py --json` 결과가 `ok: true`인지 확인한다. 실패 상태면 이번 카드 업데이트를 시도하지 않고 `ops-alerts.md`에 기록한다.
2. progress.json에서 `notion_kanban_ds`를 읽는다 (없으면 `ops-alerts.md`에 기록 후 스킵)
3. progress.json의 `kanban_card_ids[step{N}]`를 읽는다:
   - 값이 있으면 → 그 page_id로 바로 `notion-update-page` 호출 (search 생략)
   - null이면 → (예외 경로) notion-search로 카드 검색 후 진행
4. `notion-update-page`로 해당 카드의 `상태`를 "시작 전" → **"진행 중"**으로 업데이트
5. 업데이트 실패 시 `ops-alerts.md`에 기록하고, 성공한 것처럼 말하지 않는다
   - 온보딩 흐름은 계속 진행한다

#### Step 완료 시 카드 업데이트 ("완료")

각 Step의 마지막 Phase CHECK가 완료되면:
1. `scripts/check-operational-config.py --json` 결과가 `ok: true`인지 확인한다. 실패 상태면 `ops-alerts.md`에 기록하고 완료 카드 업데이트를 시도하지 않는다.
2. progress.json에서 `notion_kanban_ds`를 읽는다 (없으면 `ops-alerts.md`에 기록 후 스킵)
3. progress.json의 `kanban_card_ids[step{N}]`를 읽는다:
   - 값이 있으면 → 그 page_id로 바로 `notion-update-page` 호출 (search 생략)
   - null이면 → (예외 경로) notion-search로 카드 검색 후 진행
4. `notion-update-page`로 해당 카드의 `상태`를 "진행 중" → **"완료"**로 업데이트
5. 업데이트 실패 시 `ops-alerts.md`에 기록하고, 성공한 것처럼 말하지 않는다
   - 완료 멘트 자체는 진행하되 "Notion 반영은 HR 확인이 필요해요" 한 줄만 덧붙인다
...
- **HR Lead Slack ID**: `config/team-leads.json`의 `hr_lead.slack_id`에서 조회한다. placeholder이면 발송 금지
...
- **Step 2(Product Onboarding) 완료** → `config/team-leads.json`의 `step_domain_owners.step2_product`
- **Step 3(Biz Onboarding) 완료** → `config/team-leads.json`의 `step_domain_owners.step3_biz`
- 예시 파일의 `UXXXXXXXXXX`, `{{...}}`, `U0EXAMPLE...` 값은 실운영 ID가 아니므로 발송 금지
...
- 발송 전 `scripts/check-operational-config.py --json` 결과가 `ok: true`인지 확인한다
- **발송 도구**: `slack_send_message` MCP 사용
- **발송 실패 시**: 조용히 스킵하지 않는다. 신규입사자에게는 "Slack 알림은 HR 확인이 필요해요" 한 줄만 안내하고, `outputs/{닉네임}-{입사일}/ops-alerts.md`에 `type: slack`, 수신자 key, Step, 오류 요약을 기록한다
- 발송 실패 상태에서는 "DM을 보냈어요"라고 말하지 않는다
