...
> ⏱️ **순서 규칙(비차단)**: 신규입사자에게 첫 콘텐츠(EXPLAIN)를 **먼저** 보여준 뒤, 칸반 "진행 중" 갱신은 그 턴의 끝(또는 다음 입력 대기 직전)에 처리한다. 카드 갱신 때문에 첫 화면이 늦어지지 않게 한다. 실패 시 조용히 스킵.

각 Step 스킬이 시작되면 (첫 Phase의 EXPLAIN 전에):
1. progress.json에서 `notion_kanban_ds`를 읽는다 (없으면 스킵)
2. progress.json의 `kanban_card_ids[step{N}]`를 읽는다:
